...
public interface ResumeJpaRepository extends JpaRepository<Resume, Long> {
    ...
    // 이력서 상세보기
    @Query("select r from Resume r join fetch r.user left join fetch r.resumeSkills rs left join fetch rs.skill left join fetch r.careers where r.resumeId = :resumeId")
